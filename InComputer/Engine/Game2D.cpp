...
namespace jm
{
	...
	{
		if (isKeyPressedAndReleased(GLFW_KEY_MINUS)) draw_grid = !draw_grid;

		if (draw_grid) {
			setLineWidth(1);
			jm::drawGrid(Colors::gray, 0.2f); //Note: Game2D::drawGrid() vs jm::drawGrid(...)
			jm::drawPoint(Colors::black, vec2(0.0f, 0.0f), 5);
		}
	}
}